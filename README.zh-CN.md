...
`low`、`medium` 与 `high` 控制可见思考的长度与细致程度，每档 token 区间都能重写；每张卡片还可以独立选择温暖的 `botanical`、珍珠冰晶般的 `microglow`，或带水彩与手绘纸张气息的 `storybook` 皮肤。表达语气也可以继续改成只属于你们的版本。卡片使用兼容宿主支持的 MCP Apps UI 小组件渲染，配色、边框、字体、badge、间距、暗色模式和折叠效果全都可以自定义。你看到的不必是一块冷冰冰的日志，而可以是一张与你的聊天空间、阅读习惯和关系语气相配的思考卡片。
...
| `storybook` | 奶油画纸、水彩鼠尾草、杏色、月夜梅紫 | 手绘边框、水彩花影与安静的绘本气息 |

`skin` 在每次工具调用中独立选择，不会与 `style` 或 `effort` 绑定。用户明确指定时优先遵循；未指定时模型可以选择最适合本轮氛围的皮肤。三套皮肤都包含跟随宿主的暗色模式。
