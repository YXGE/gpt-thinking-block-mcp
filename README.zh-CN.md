...
`low` 到 `max` 控制可见思考的长度与细致程度，每档 token 预算都能重写；表达语气也可以继续改成只属于你们的版本。卡片使用兼容宿主支持的 MCP Apps UI 小组件渲染，配色、边框、字体、badge、间距、暗色模式和折叠效果全都可以自定义。你看到的不必是一块冷冰冰的日志，而可以是一张与你的聊天空间、阅读习惯和关系语气相配的思考卡片。
...
| effort | 软目标 | 适合的情况 |
|---|---:|---|
| `low` | 约 150 tokens | 快速检查 |
| `medium` | 约 600 tokens | 一般分析 |
| `high` | 约 2,000 tokens | 困难决策或研究 |
| `max` | 约 6,000 tokens | 长篇探索 |

这些是给模型的生成目标，不是服务端硬性限制。如果想严格控制长度，可以看后面的[调整 thinking 长度](#调整-thinking-长度)。
...
修改 `effort` 字段 description 中的目标长度即可。目前是软目标。如果想设置硬上限，可以在 `handle()` 里验证或截断 `thinking`；更建议返回清楚的工具错误，不要在没有提示的情况下偷偷截断。
